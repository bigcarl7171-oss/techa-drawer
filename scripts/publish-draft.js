...
// 인라인 서식: **굵게** 와 내부링크 [글자](/경로) 만 받는다.
// 링크를 2026-09-19 에 추가했다 — 그전까지 변환기가 링크를 몰라서 S3(나가는 링크)를
// 매번 발행본 HTML 에 손으로 넣어야 했고, publish-draft 를 재실행하면 지워졌다.
// 목적지는 `/` 로 시작하는 내부 경로만 허용한다. 외부 링크는 초안에서 새어 들어오면
// 안 되고(네이버판은 techa.kr 링크 자체가 금지), 매거진 본문의 외부 링크는 판단이
// 필요한 일이라 스크립트가 조용히 통과시킬 것이 아니다.
function inline(s) {
  return esc(s)
