...
4. **변경률 10~25%를 목표로 수술한다** — **30% 초과는 경고**, **50% 초과는 강제 중단·롤백**. 멀쩡한 사람 글을 평균값으로 깎아내는 게 가장 흔한 실패다.
...
0. **트리아지** — 무엇을 어디까지 고칠지 먼저 정한다.
   - 흔적이 무더기로 겹칠 때만 문장 단위로 손댄다 (줄표 하나, 접속어 하나는 그냥 둔다).
...
한글 + 괄호 영어를 매번 병기("주권 AI(Sovereign AI)" 반복)하거나, 옮길 수 있는 영어를 그대로 둔다. 첫 등장만 병기하고 이후 한글만. 단, 업계 표준 약어는 유지("원문 그대로 보존할 것" 참고).
