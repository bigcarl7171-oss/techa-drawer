...
- ⭐ **네이버 블로그가 본편이다 (2026-09-19).** 네이버 통합검색에는 자사 도메인이 들어갈
  자리가 없다 — 실측에서 techa.kr은 0건이고 화면은 블로그·카페가 채운다. 매거진은
  구글·브랜드 자산으로 계속 쓰되 **작성 순서에서 뒤에 둔다.**
...
- **목록은 손으로 고치지 않는다** — 홈 도구 표·관련 도구·홈 검색 목록은 생성기 3종이
  찍는다(`build-home-tools.js` · `build-related.js` · `build-posts.js`). `site.js` 나
