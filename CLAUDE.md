...
| 2 | **사람이 부름** | "초안 써줘" → `routine-draft.md` 대로 스크래치 폴더(`C:\연습\<slug>_원고\`)에 **`blog.md`만** 쓴다. `magazine.md`는 아직 안 쓴다. 저장소엔 아무것도 안 만든다 |
| 3 | **사람** | `blog.md` 검토·수정 (채팅에서 반복) → 확정되면 **승인** ("승인" / "발행해줘") |
| 4 | **승인하면 이어서** | `.claude/skills/techa-publish` 실행 — 승인된 `blog.md`로 **`magazine.md`를 그때 새로 씀**(내용은 같게, 문장은 일치하지 않게) → 사진 적용 → 직전 발행본 archive → `docs/drafts/<날짜>-<slug>.md`로 들여옴 → 발행본 → 검증 → push(=배포) → topic-pool·INDEX 갱신. 중간에 멈추지 않는다 (2026-09-24 개정) |
