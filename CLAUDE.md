...
| 시각 | 누가 | 무엇을 |
|---|---|---|
| 04:00 KST | 초안 루틴 | 주제 3안 선정 → 1안으로 초안 작성 → `docs/drafts/`에 커밋 (`status: draft`) |
| 아침 | **사람** | ①원고 검토·수정 ②사진을 `docs/drafts/images/<슬러그>/`에 넣기 ③`status: ready`로 바꿔 커밋 |
...
- ⚠️ 사진을 직접 넣었으면 `status: ready`로 바꿀 때 압축본 `blog/<슬러그>/*.jpg`도 **같이 커밋**해야 한다. 원본 폴더는 gitignore라 클라우드 루틴이 못 보고, 그냥 두면 AI 컷으로 조용히 덮어쓴다.
